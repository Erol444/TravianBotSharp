    ...
    public static class TaskExecutor
    {
        ...
        public static async Task PageLoaded(Account acc)
        {
            ...
            if (acc.AccInfo.Tribe == null && CheckSkipTutorial(acc))
                await DriverHelper.ExecuteScript(acc,
                    "document.getElementsByClassName('questButtonSkipTutorial')[0].click();");
        ...
        public static async Task Execute(Account acc, BotTask task)
        {
            ...
            try
            {
                acc.Wb.Log($"Executing task {task.GetName()}" +
                           (task.Vill == null ? "" : $" in village {task.Vill.Name}"));
        ...
        public static void UpdateDorf1Info(Account acc)
        {
            ...
            if (dorf1Movements.Any(x => x.Type == Classificator.MovementTypeDorf1.IncomingAttack) &&
                vill.Deffing.AlertType != AlertTypeEnum.Disabled)
        ...
        {
            foreach (var task in tasks) acc.Tasks.Add(task);
            ReorderTaskList(acc);
        }
        ...
        {
            if (!TaskExistsInVillage(acc, vill, task.GetType())) AddTask(acc, task);
        }

        public static bool TaskExistsInVillage(Account acc, Village vill, Type taskType)
        {
            return acc.Tasks.Any(x => x.GetType() == taskType && x.Vill == vill);
        }
        ...
        {
            return acc.Wb.Html.DocumentNode.Descendants().Any(x => x.HasClass("questButtonSkipTutorial"));
        }

        private static bool CheckContextualHelp(Account acc)
        {
            return acc.Wb.Html.GetElementbyId("contextualHelp") != null;
        }

        /// <summary>
        ///     Checks if account is banned (T4.5)
        /// </summary>
        private static bool IsBanMsg(Account acc)
        {
            return acc.Wb.Html.GetElementbyId("punishmentMsgButtons") != null;
        }
        ...
        {
            return acc.Wb.Html.GetElementbyId("CybotCookiebotDialogBodyLevelButtonLevelOptinDeclineAll") != null;
        }

        private static bool CheckCookiesNew(Account acc)
        {
            return acc.Wb.Html.DocumentNode.Descendants("a")
                .Any(x => x.HasClass("cmpboxbtn") && x.HasClass("cmpboxbtnyes"));
        }

        private static bool IsCaptcha(Account acc)
        {
            return acc.Wb.Html.GetElementbyId("recaptchaImage") != null;
        }
