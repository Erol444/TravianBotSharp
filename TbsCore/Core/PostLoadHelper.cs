...
namespace TbsCore.Helpers
{
    ...
    internal class PostLoadHelper
    {
        ...
        public static List<Action> GetPostLoadTasks(Account acc)
        {
            ...
            {
                // 1:
                () => acc.AccInfo.ServerVersion = acc.Wb.Html.GetElementbyId("sidebarBoxDailyquests") == null
                    ? Classificator.ServerVersionEnum.T4_5
                    : Classificator.ServerVersionEnum.T4_4,
                // 2:
                () =>
                {
                ...
                () =>
                {
                        ...
                        villExpansionReady != null)
                    {
                        ...
                        TaskExecutor.AddTaskIfNotExists(acc,
                            new SendSettlers {ExecuteAt = DateTime.Now, Vill = villExpansionReady});
                    }
                },
                // 5. Beginner Quests:
                () =>
                ...
                // 13:
                () => vill.Timings.NextVillRefresh =
                    DateTime.Now.AddMinutes(ran.Next(vill.Settings.RefreshMin, vill.Settings.RefreshMax)),
                // 14 NPC:
                () =>
                {
                    var ratio = (float) vill.Res.Stored.Resources.Crop / vill.Res.Capacity.GranaryCapacity;
                    if (0.99 <= ratio &&
                        3 <= acc.AccInfo.Gold &&
                ...
                () =>
                {
                    var heroReady = acc.Hero.HeroInfo.Health > acc.Hero.Settings.MinHealth &&
                    ...
                    if (homeVill == null)
                        TaskExecutor.AddTask(acc, new HeroUpdateInfo {ExecuteAt = DateTime.Now});
                             ...
                             homeVill.Build.Buildings.Any(x =>
                                 x.Type == Classificator.BuildingEnum.RallyPoint && 0 < x.Level) &&
                             (acc.Hero.AdventureNum != acc.Hero.Adventures.Count() || HeroHelper.AdventureInRange(acc)))
                        // Update adventures
                        TaskExecutor.AddTaskIfNotExists(acc,
                            new StartAdventure {ExecuteAt = DateTime.Now.AddSeconds(10)});
                    if (acc.Hero.AdventureNum == 0 && acc.Hero.Settings.BuyAdventures) //for UNL servers, buy adventures
                        TaskExecutor.AddTaskIfNotExists(acc,
                            new TTWarsBuyAdventure {ExecuteAt = DateTime.Now.AddSeconds(5)});
                    if (acc.Hero.Status == Hero.StatusEnum.Dead &&
                        acc.Hero.Settings.AutoReviveHero) //if hero is dead, revive him
                ...
                () =>
                {
                    if (acc.Settings.ExtendProtection &&
                        acc.Wb.Html.GetElementbyId("sidebarBoxInfobox").Descendants("button")
                            .Any(x => x.GetAttributeValue("value", "") == "Extend"))
                        // infoType_25 ?
        ...
        {
            var boostProduction = acc.Wb.Html.DocumentNode.Descendants("button")
                .FirstOrDefault(x => x.HasClass("productionBoostButton"));
        ...
        private static void AutoExpandStorage(Account acc, Village vill)
        {
            ...
            var warehouse_delta = vill.Res.Capacity.WarehouseCapacity * 0.95;
            var granary_delta = vill.Res.Capacity.GranaryCapacity * 0.95;
