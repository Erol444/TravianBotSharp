...
namespace TravBotSharp.Files.Tasks.LowLevel
{
    public class ResearchTroop : BotTask
    {
        ...
        {
            if (!await VillageHelper.EnterBuilding(acc, Vill, BuildingEnum.Academy))
            ...
            if (button == null) RepeatTask(Vill, troop, DateTime.Now);
            var (dur, cost) = TroopsParser.AcademyResearchCost(acc.Wb.Html, troop);
            ...
            if (Vill.Settings.AutoImprove)
                TaskExecutor.AddTask(acc, new ImproveTroop {Vill = Vill, ExecuteAt = DateTime.Now.Add(dur)});
        ...
        private void RepeatTask(Village vill, TroopsEnum troop, DateTime nextExecute)
        {
            vill.Troops.ToResearch.Remove(troop);
            //Next research when this one finishes
            if (0 < vill.Troops.ToResearch.Count) NextExecute = nextExecute;
        }
    }
}