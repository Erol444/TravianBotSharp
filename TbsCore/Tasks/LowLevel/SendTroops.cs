...
namespace TravBotSharp.Files.Tasks.LowLevel
{
    public class SendTroops : BotTask
    {
        ...
        public override async Task<TaskRes> Execute(Account acc)
        {
            ...
            if (TroopsMovement.Troops.Sum() == 0) return TaskRes.Executed;

            // Add number of troops to the input boxes
            for (var i = 0; i < TroopsMovement.Troops.Length; i++)
            {
            ...
            var script =
                $"Array.from(document.getElementsByName('c')).find(x=>x.value=={(int) TroopsMovement.MovementType}).checked=true;";
            ...
            if (TroopsMovement.Target1 != Classificator.BuildingEnum.Site)
                await DriverHelper.SelectIndexByName(acc, "ctar1", (int) TroopsMovement.Target1);
            if (TroopsMovement.Target2 != Classificator.BuildingEnum.Site)
                await DriverHelper.SelectIndexByName(acc, "ctar2", (int) TroopsMovement.Target2);

            // Scout type
            if (TroopsMovement.ScoutType != ScoutEnum.None)
            {
                var scout =
                    $"Array.from(document.getElementsByName('spy')).find(x=>x.value=={(int) TroopsMovement.ScoutType}).checked=true;";
