    ...
    public class SendFarmlist : BotTask
    {
        ...
        public override async Task<TaskRes> Execute(Account acc)
        {
            ...
            if (flNode == null)
            {
                ...
                if (mainVill == Vill) return TaskRes.Executed; // No gold account?
            ...
            if (acc.Farming.TrainTroopsAfterFL) // For TTWars servers
                TaskExecutor.AddTask(acc, new TrainTroops
                {
                    ...
                    Troop = Vill.Troops.TroopToTrain ?? TroopsEnum.Hero,
                    Vill = Vill,
            ...
            {
                await DriverHelper.ExecuteScript(acc, $"Travian.Game.RaidList.toggleList({FL.Id});");
                // Update flNode!
                flNode = GetFlNode(acc.Wb.Html, acc.AccInfo.ServerVersion);
            }
            ...
            foreach (var farm in flNode.Descendants("tr").Where(x => x.HasClass("slotRow")))
            {
                ...
                if (img.Count() != 0 && ( //no image -> no recent attack
                        img.FirstOrDefault(x => x.HasClass("iReport3")) != null &&
                        FL.RaidStyle != RaidStyle.RaidLost //raid was lost and we don't have RaidLost raidstyle
                        || img.FirstOrDefault(x => x.HasClass("iReport2")) != null &&
                        FL.RaidStyle == RaidStyle.RaidSuccessful //some casualities, but we only attack on successful
                    ))
                    continue;
            ...
            acc.Wb.Log($"FarmList '{FL.Name}' was sent");
        ...
        private HtmlNode GetFlNode(HtmlDocument htmlDoc, ServerVersionEnum version)
        {
            switch (version)
            {
                case ServerVersionEnum.T4_4: return htmlDoc.GetElementbyId("list" + FL.Id);
                case ServerVersionEnum.T4_5: return htmlDoc.GetElementbyId("raidList" + FL.Id);
                default: return null;
            }
        }
    }
