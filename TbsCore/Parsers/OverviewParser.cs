...
namespace TravBotSharp.Files.Parsers
{
    ...
    {
        public static void UpdateTroopsLevels(HtmlDocument htmlDoc, ref Account acc)
        {
            var table = htmlDoc.DocumentNode.Descendants("table").FirstOrDefault(x => x.HasClass("under_progress"));

            // Go through all villages
            foreach (var villTr in table.ChildNodes.First(x => x.Name == "tbody").ChildNodes.Where(x => x.Name == "tr")
                .ToList())
            {
                var villId =
                    Convert.ToInt32(villTr.Descendants("a").FirstOrDefault().GetAttributeValue("href", "").Split('=')[1]
                        .Split('&')[0]);
