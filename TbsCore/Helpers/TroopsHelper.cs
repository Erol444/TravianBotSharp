...
namespace TravBotSharp.Files.Helpers
{
    public static class TroopsHelper
    {
        ...
        internal static long TroopsToFill(Account acc, Village vill, TroopsEnum troop, bool great)
        {
            ...
            var trainBuilding = GetTroopBuilding(troop, great);
            var trainingTime = GetTrainingTimeForBuilding(trainBuilding, vill);
        ...
        public static void ReStartTroopTraining(Account acc, Village vill)
        {
            ...
            if (vill.Settings.BarracksTrain != TroopsEnum.None &&
                !vill.Troops.ToResearch.Any(x => x == vill.Settings.BarracksTrain))
            {
                var barracksTrain = DateTime.Now;
                if (vill.Troops.CurrentlyTraining.Barracks.Count > 0)
                    barracksTrain = vill.Troops.CurrentlyTraining.Barracks.Last().FinishTraining
                        .AddHours(-acc.Settings.FillInAdvance);
                ...
                if (vill.Settings.GreatBarracksTrain)
                {
                    ...
                    if (vill.Troops.CurrentlyTraining.GB.Count > 0)
                        gbTrain = vill.Troops.CurrentlyTraining.GB.Last().FinishTraining
                            .AddHours(-acc.Settings.FillInAdvance);
                    TaskExecutor.AddTask(acc, new TrainTroops
                    {
                        ExecuteAt = gbTrain,
            ...
            //stable
            if (vill.Settings.StableTrain != TroopsEnum.None &&
                !vill.Troops.ToResearch.Any(x => x == vill.Settings.StableTrain))
            {
                var stableTrain = DateTime.Now;
                if (vill.Troops.CurrentlyTraining.Stable.Count > 0)
                    stableTrain = vill.Troops.CurrentlyTraining.Stable.Last().FinishTraining
                        .AddHours(-acc.Settings.FillInAdvance);
                TaskExecutor.AddTask(acc, new TrainTroops
                {
                ...
                if (vill.Settings.GreatStableTrain)
                {
                    ...
                    if (vill.Troops.CurrentlyTraining.GS.Count > 0)
                        gsTrain = vill.Troops.CurrentlyTraining.GS.Last().FinishTraining
                            .AddHours(-acc.Settings.FillInAdvance);
                    TaskExecutor.AddTask(acc, new TrainTroops
                    {
                        ExecuteAt = gsTrain,
            ...
            //workshop
            if (vill.Settings.WorkshopTrain != TroopsEnum.None &&
                !vill.Troops.ToResearch.Any(x => x == vill.Settings.WorkshopTrain))
            {
                var wsTrain = DateTime.Now;
                if (vill.Troops.CurrentlyTraining.Workshop.Count > 0)
                    wsTrain = vill.Troops.CurrentlyTraining.Workshop.Last().FinishTraining
                        .AddHours(-acc.Settings.FillInAdvance);
                TaskExecutor.AddTask(acc, new TrainTroops
                {
                    ExecuteAt = wsTrain,
        ...
        {
            var fillTimes = new List<DateTime>();
            // Check barracks
            if (vill.Settings.BarracksTrain != TroopsEnum.None &&
                !vill.Troops.ToResearch.Any(x => x == vill.Settings.BarracksTrain))
            {
            ...
            for (var i = 0; i < fillTimes.Count; i++)
        ...
        {
            TroopsEnum[] troops = {vill.Settings.BarracksTrain, vill.Settings.StableTrain, vill.Settings.WorkshopTrain};
            foreach (var troop in troops)
            {
                ...
                {
                    if (AddBuildingsForTroop(acc, vill, troop))
                    {
                ...
                if (!vill.Troops.Levels.Exists(x => x.Troop == troop && x.Level == 20) && vill.Settings.AutoImprove)
                {
                    ...
                    if (vill.Build.Buildings.Any(x => x.Type == BuildingEnum.Smithy))
                        TaskExecutor.AddTaskIfNotExistInVillage(acc, vill,
                            new ImproveTroop {Vill = vill, ExecuteAt = DateTime.Now});
                }
                else
                {
        ...
        /// <summary>
        ///     When user wants to train a new troop, we need to first upgrade buildings required and then research+improve the
        ///     troop
        /// </summary>
        /// <param name="acc">Account</param>
