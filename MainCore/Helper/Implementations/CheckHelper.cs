...
namespace MainCore.Helper.Implementations
{
    public class CheckHelper : ICheckHelper
    {
        ...
        private readonly IUpgradingPageParser _upgradingPageParser;
        private readonly IUrlValidator _urlValidator;
        private readonly ISystemPageParser _systemPageParser;

        public CheckHelper(IChromeManager chromeManager, IVillagesTableParser villagesTableParser, IBuildingTabParser buildingTabParser, IUpgradingPageParser upgradingPageParser, IDbContextFactory<AppDbContext> contextFactory, IUrlValidator urlValidator, ISystemPageParser systemPageParser)
        {
            _chromeManager = chromeManager;
            _villagesTableParser = villagesTableParser;
            _buildingTabParser = buildingTabParser;
            _upgradingPageParser = upgradingPageParser;
        ...
        public int[] GetResourceNeed(int accountId, BuildingEnums building, bool multiple = false)
        {
            ...
            {
                contractNode = _upgradingPageParser.GetContractNode(html);
            }
        ...
        public Result<bool> IsNeedAdsUpgrade(int accountId, int villageId, PlanTask buildingTask)
        {
            ...
            {
                return Result.Fail("Cannot found duration in build page. (div)");
            }
            var dur = durationNode.Descendants("span").FirstOrDefault(x => x.HasClass("value"));
            if (dur is null)
            {
                return Result.Fail("Cannot found duration in build page. (span)");
            }
        ...
        public bool IsFarmListPage(int accountId)
        {
            ...
            if (!_urlValidator.IsFarmList(url)) return false;
